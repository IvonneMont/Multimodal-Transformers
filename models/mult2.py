...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x_in, x_in_k = None, x_in_v = None):
        ...
        if self.embed_positions is not None:
            xp=x_in.transpose(0, 1)[:, :, 0]
            y=self.embed_positions(xp).transpose(0, 1)   # Add positional embedding
            x+=y
...
class MULTModel2(nn.Module):
    def __init__(self, hyp_params):
        ...
        if self.partial_mode == 1:
            combined_dim = 2 * self.d_l   # assuming d_l  == d_v
        else:
            combined_dim = self.d_l  + self.d_v
        
        output_dim = hyp_params.n_classes      # This is actually not a hyperparameter :-)
